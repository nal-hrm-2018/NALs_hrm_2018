    <div class="form-group col-md-4">
        ...
        <select name="month_absence" id="month-absence" class="form-control">
            <option {{ !empty(old('month_absence'))?'':'selected="selected"' }} value="">
                {{  trans('vendor.drop_box.placeholder-default') }}
            </option>
            @foreach($month_absences as $key=>$value)
                <option value="{{ $key }}" {{ (string)$key===old('month_absence')?'selected="selected"':'' }}>
                    {{ $value }}
                </option>
            @endforeach
        </select>
    ...
    <div class="form-group col-md-4">
        ...
        <select name="year_absence" id="year-absence" class="form-control">
            <option {{ !empty(old('year_absence'))?'':'selected="selected"' }} value="">
                {{  trans('vendor.drop_box.placeholder-default') }}
            </option>
            @foreach($year_absences as $key=>$value)
                <option value="{{ $key }}" {{ (string)$key===old('year_absence')?'selected="selected"':'' }}>
                    {{ $value }}
                </option>
            @endforeach
        </select>
